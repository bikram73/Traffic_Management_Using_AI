...
class Vehicle(pygame.sprite.Sprite):
    def __init__(self, lane, vehicleClass, direction_number, direction, turn_direction):
        ...
        self.crossed = 0
        self.turnDirection = turn_direction   # -1 = left, 0 = straight, 1 = right
        self.willTurn = 1 if turn_direction != 0 else 0
        self.turned = 0
        self.rotateAngle = 0
                ...
                if(self.crossed==0 or self.x+self.currentImage.get_rect().width<mid[self.direction]['x']):
                    ...
                    if(self.turned==0):
                        ...
                        if(self.index==0 or self.y>(vehicles[self.direction][self.lane][self.index-1].y + vehicles[self.direction][self.lane][self.index-1].currentImage.get_rect().height + gap2)):
                            self.x = move_towards(self.x, x['up'][0], self.speed)
                            self.y -= self.speed
            else:
                ...
                if(self.crossed==0 or self.y+self.currentImage.get_rect().height<mid[self.direction]['y']):
                    ...
                    if(self.turned==0):
                        ...
                        if(self.index==0 or self.x+self.currentImage.get_rect().width<(vehicles[self.direction][self.lane][self.index-1].x - gap2)):
                            self.y = move_towards(self.y, y['right'][0], self.speed)
                            self.x += self.speed
            else:
                ...
                if(self.crossed==0 or self.x>mid[self.direction]['x']):
                    ...
                    if(self.turned==0):
                        ...
                        if(self.index==0 or self.y+self.currentImage.get_rect().height<(vehicles[self.direction][self.lane][self.index-1].y - gap2)):
                            self.x = move_towards(self.x, x['down'][0], self.speed)
                            self.y += self.speed
            else:
                ...
                if(self.crossed==0 or self.y>mid[self.direction]['y']):
                    ...
                    if(self.turned==0):
                        ...
                        if(self.index==0 or self.x>(vehicles[self.direction][self.lane][self.index-1].x + vehicles[self.direction][self.lane][self.index-1].currentImage.get_rect().width + gap2)):
                            self.y = move_towards(self.y, y['left'][0], self.speed)
                            self.x -= self.speed
            else:
...
def announce_detection(direction_name):
    # `say` exists on macOS; skip on other platforms.
    if shutil.which("say"):
        os.system(f"say detecting vehicles, {direction_name}")

# Set time according to formula
def setTime():
    global noOfCars, noOfBikes, noOfBuses, noOfTrucks, noOfRickshaws, noOfLanes
    global carTime, busTime, truckTime, rickshawTime, bikeTime
    announce_detection(directionNumbers[(currentGreen+1)%noOfSignals])
...
def repeat():
    ...
    while(signals[currentGreen].green>0 and not stopSimulation.is_set()):   # while the timer of current green signal is not zero
        printStatus()
        updateValues()
    ...
    while(signals[currentGreen].yellow>0 and not stopSimulation.is_set()):  # while the timer of current yellow signal is not zero
        printStatus()
        updateValues()
        time.sleep(1)
    if stopSimulation.is_set():
...
def generateVehicles():
    while(not stopSimulation.is_set()):
        vehicle_type = random.randint(0,4)
        lane_number = random.randint(0,2)
        # Dedicated lane behavior keeps vehicles in distinct road paths.
        ...
        if(temp<a[0]):
            ...
            direction_number = 3
        Vehicle(lane_number, vehicleTypes[vehicle_type], direction_number, directionNumbers[direction_number], turn_direction)
        time.sleep(0.25)
